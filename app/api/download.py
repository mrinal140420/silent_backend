...
def download_file(
    ...
    if not allow_token(token):
        raise HTTPException(status_code=429, detail="Too many requests for this link")

    # -------------------------
    # Access link validation
    # -------------------------
    link = access_links_col.find_one({"token": token, "revoked": False})
    if not link:
        raise HTTPException(status_code=404, detail="Invalid link")

    file = files_col.find_one({"file_id": link["file_id"]})
    if not file:
        raise HTTPException(status_code=410, detail="File destroyed")

    # -------------------------
    # Password enforcement
    # -------------------------
    if link.get("password_hash"):
        if not verify_password(password or "", link["password_hash"]):
    ...
    if updated["download_count"] >= updated["max_downloads"]:
        background_tasks.add_task(destroy_file, updated["file_id"])
